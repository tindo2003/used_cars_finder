...
- [x] **Private-party vs. dealer distinction in the comparable pool** — fixed 2026-07-20, same day as re-flagging with concrete evidence (a Lexus-grouped digest dominated by very old, very cheap Craigslist listings). `_is_comparable()` now requires matching `seller_type`. Full detail: [deal-scoring-heuristic.md](./deal-scoring-heuristic.md).
...
- [x] **Fixed a live crash: cross-dealer inventory-link collisions on `original_url`** — found 2026-07-20 from a real GitHub Actions failure (`duplicate key value violates unique constraint "listings_original_url_key"`) while scraping Capitol Honda. Root cause: `providers/dealerinspire.py` takes `original_url` verbatim from the card's own `href`, and that href can point straight to a *different* dealer's site (confirmed live: a Capitol Honda card linking to `chevroletoffremont.com` — the same physical vehicle, shared/syndicated between two unrelated-looking dealers). `original_url` has its own table-wide unique constraint independent of `(vin, dealer_name)`, so upserting on `(vin, dealer_name)` (a new pair) can still crash on `original_url` colliding with someone else's existing row. `upsert()` now catches that specific constraint violation and skips the car entirely (returns `None`, tallied in a new `progress["skipped"]` counter) rather than overwriting the existing row's identity through a side door — the same class of bug `(vin, dealer_name)` was built to prevent, just tripped via a different column. Verified against production: reproduced the exact crash with synthetic data, confirmed it now skips cleanly with the existing row untouched, cleaned up after. 3 new tests.
- [x] **Geocoding listings into the `location` PostGIS column** — built and backfilled live 2026-07-21. `scraper/geocoding.py` geocodes each distinct `city` string via Nominatim/OpenStreetMap and writes the result back as a `geometry(Point,4326)`, run as a new step in `notify.py`'s daily pipeline. 980 of 2052 active listings backfilled; the rest are Craigslist rows whose scraped location text isn't a real place name (phone numbers/CTAs, multi-neighborhood slash lists), left NULL rather than guessed. See `memory.md` for full detail. **The radius search filter itself [4.2, 5.4] is still open** (frontend UI + RPC + saved-search matching) — this item only closes the data gap that was blocking it.
- [ ] Unit tests for provider parsing logic (dealeron/dealerinspire/craigslist `extract_*` functions) using fixture HTML
- [x] **Lexus Stevens Creek's DealerOn provider undercounts inventory** — diagnosed and fixed 2026-07-20. Real cause: this dealer's vehicle-card grid renders virtualized (cards for the current page mount progressively as you scroll, from data already fetched in one API call — confirmed live via `performance.getEntriesByType('resource')` showing zero new network requests while the rendered count climbed 4→24). A single Playwright snapshot right after `networkidle` only ever captured whatever rendered first. Fixed with `providers/dealeron.py`'s `_scroll_until_stable()`, which scrolls to the bottom repeatedly until the rendered count stops growing, called once per page before parsing; costs ~0 overhead on non-virtualized dealers. 3 new tests in `tests/test_dealeron.py`.
...
- [x] Batched digest emails via Resend — one email per saved search covering its top N matches (configurable `--notify-top-n`), not one email per listing
...
- [x] Verified live: real Resend emails sent for a real saved search
...
- [x] **Digest always sends today's top N, not just novel matches** — resolved 2026-07-20, replacing the old dedup design. Previously, `notification_history` (unique `(saved_search_id, listing_id)`) permanently excluded a listing from future digests once notified once, meaning an already-notified listing never re-surfaced even after a material price drop or other improvement (found as a real gap, external review 2026-07-19). Explicit user call: they'd rather get "the top N good deals right now" every day, resends included, than build price-drop-detection logic to selectively re-trigger. `notify_matches()` no longer reads `notification_history` to filter; it always ranks and sends the current top N. `notification_history` is kept only as a write-only log of what was sent when (for a possible future click-through-rate metric per PRD section 9), so migration 013 dropped its uniqueness constraint — it now allows more than one row per `(saved_search_id, listing_id)` pair, one per day that pair appears in a digest.
- [x] **Grouped digests by make/model** — built and verified live 2026-07-20 (migration 014, `notification_grouping` column: `combined` default / `make` / `model`). A saved search watching several makes/models can split its digest into one email per distinct make/model among its matches, each independently capped to `top_n` (not a shared `top_n` split across groups) — confirmed live: a real digest run produced 2 separate emails for a Toyota+Lexus search grouped by make.
- [x] **Digest emails identify which saved search they're for** — fixed 2026-07-20 same day, after user feedback on real production emails ("Today's top matches for your saved search" was too generic to tell 2 simultaneous emails apart). Subject/intro now prefer the search's name, falling back to a make/model description (`notifications._search_label`).
- [ ] Browser push notifications — not built (email-only; sufficient for single-user/personal use so far)
- [ ] **Notification preferences** (new, external review 2026-07-19) — no user-configurable frequency or unsubscribe flow yet. User-accepted gap, not currently planned.
...
- [x] **Transmission filter [4.2] and Seller type filter [4.2]** — built 2026-07-20. Backend (`SavedSearch`/`matches()`) already supported both as scalar exact-match filters; this was purely a frontend addition (dropdowns + query-building), no migration needed — confirmed live that `saved_searches.transmission`/`seller_type` columns already existed in production. Options are built from real distinct values in `listings` (never a hardcoded guess), so Seller Type only ever offers "dealer" (Craigslist/eBay don't expose an owner/dealer distinction) and Transmission reflects each dealer feed's actual wording verbatim. Transmission options narrow to the selected make(s)/model(s), same pattern as Model-under-Make. Also added an "Any Automatic" quick-pick (substring match on "automatic") since dealer feeds store transmission as a highly specific manufacturer string ("6-Speed Automatic ECT-i", "9-Speed 948TE Automatic") rather than a clean Automatic/Manual split — a literal, not semantic, bucket by explicit user choice (CVT/dual-clutch transmissions stay outside it).
- [ ] Search radius filter [4.2] — the geocoding gap that blocked this is now closed (see Data ingestion above); still need the frontend UI, a PostGIS RPC wired into `buildListingsQuery`, and radius-matching in `notifications.matches()`
...
| Apply filters | 🟡 partial (6 of 7 PRD filters incl. transmission/seller type, 2026-07-20; radius filter still unbuilt, though its geocoding data-gap closed 2026-07-21) |
...
6. ~~Notification gaps: always-resend-top-N digest~~ — done, see Notification/matching engine above. Preferences/unsubscribe remains an accepted, not-currently-planned gap.
7. ~~Multi-make/multi-model search + grouped notification digests~~ — done, see Search & filters / Notification engine above.
8. ~~Remaining filters (transmission, seller type)~~ — done, see Search & filters above. **Listing detail page still open** — lower priority for this audience than for a general-purpose shopper, and scope needs revisiting: the PRD's "full description, more photos" isn't backed by any currently-scraped data (every provider collects exactly one photo per vehicle, and `description` was never scraped from any source — would need new per-vehicle-page scraping work, not just new frontend UI). A detail page today could only show fields already on the card plus `trim`/`vin`/`fuel_type`/`posted_at`, which do exist but aren't rendered anywhere yet.
9. ~~Geocoding for radius search~~ — done, see Data ingestion above (2026-07-21). **Radius search filter itself (frontend + RPC + saved-search matching) and additional Bay Area dealer coverage remain open** — expand breadth once the core loop is fully tuned. Additional marketplace *types* (Cars.com, Autotrader, Facebook Marketplace) rank below this for a deal-hunter audience.
