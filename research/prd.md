...
**Escalated to highest priority, then resolved (2026-07-19):** external review independently flagged this same gap: the PRD repeatedly promises "good deals" (1.1, 2.1) but the functional requirements (4.1–4.7) only described filter-matching, not deal quality. **Now built** (`scraper/deals.py`): a listing's price is compared against the median of other active listings with the same make/model within ±2 model years and ±20,000 miles (requires at least 3 comparables to trust the median); a listing counts as a good deal at 12% or more below that median. Notification digests now rank by this score (best relative deal first) instead of plain lowest price, falling back to price when there isn't enough comparable data to score a listing. This is deliberately crude — no trim-level distinction, no condition/accident history — and is a minimal signal, not the full Deal Score in the Future Roadmap (section 8).
