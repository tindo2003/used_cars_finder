...
See `research/mvp-checklist.md`'s "Suggested build order" for the live, prioritized list. As of this writing, next up: **notification gaps** (updated-listing/price-drop re-triggering, per-user preferences/unsubscribe). Also open: saved-search edit/enable-disable, several PRD filters (transmission, seller type, radius search -- blocked on geocoding), a listing detail page, additional dealer coverage. All migrations through 011 have been run in Supabase.
